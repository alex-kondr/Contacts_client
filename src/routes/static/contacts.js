...
const getContacts = async (access_token=localStorage.getItem("access_token"), refresh_token=localStorage.getItem("refresh_token")) => {
    ...
    const response = await fetch(
        'https://Contactsserver.olieksandrkond3.repl.co/api/contacts/?limit=100',
        {
            method: 'GET',
            headers: {
...
const getNewTokens = async (refresh_token) => {
    const response = await fetch(
        'https://Contactsserver.olieksandrkond3.repl.co/api/auth/refresh_token',
            {
                method: 'GET',
                headers: {
...
const deleteContact = async (contactId) => {
    const response = await fetch(
        `https://Contactsserver.olieksandrkond3.repl.co/api/contacts/${contactId}`,
            {
                method: 'DELETE',
                headers: {
...
const deleteAvatar = async (contactId) => {
    const response = await fetch(
        `https://Contactsserver.olieksandrkond3.repl.co/api/contacts/avatar/${contactId}`,
        {
            method: 'DELETE',
            headers: {
                Authorization: `Bearer ${localStorage.getItem("access_token")}`
